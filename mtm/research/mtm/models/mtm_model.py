...
class MTM(nn.Module):
    ...
    def trajectory_encoding(self, trajectories) -> Dict[str, torch.Tensor]:
        ...
        for key, traj in trajectories.items():
            # print(f"shapes {traj.shape} {self.encoder_embed_dict[key](traj.to(torch.float32)).shape}")
            # print(f"shapes {self.encoder_per_dim_encoding[key].shape}")
            # print(f"shapes {self.pos_embed[:, : traj.shape[1], :, :].shape}")
            encoded_traj = (
